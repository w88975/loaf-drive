...
import { NewFolderModal, RenameModal, DeleteModal, MoveModal, PasswordModal, ShareModal, ShareResultModal } from '../components/overlays/Modals';
...
export const FilesView: React.FC<FilesViewProps> = ({ 
  ...
  const [activeModal, setActiveModal] = useState<'new-folder' | 'rename' | 'delete' | 'move' | 'password-enter' | 'password-unlock' | 'share' | null>(null);
  ...
  const handlePasswordConfirm = (password: string) => {
    if (activeModal === 'password-enter') {
      ...
      toggleLock.mutate({ id: targetItem.id, isLocked: false, password }, { onSuccess: () => { removeCachedPassword(targetItem.id); setActiveModal(null); } });
    }
  };
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-4 md:p-6 pb-2 flex flex-wrap gap-2">
        <button onClick={onUploadClick} className="flex items-center space-x-2 bg-black text-white px-3 md:px-4 py-2 hover:bg-yellow-400 hover:text-black border-2 border-black text-[10px] md:text-xs font-bold uppercase transition-colors"><Icons.Plus className="w-3 h-3" /><span>Upload</span></button>
        <NewFolderAction onClick={() => setActiveModal('new-folder')} />
      </div>

      <div className="flex-1 overflow-y-auto p-4 md:p-6" onContextMenu={handleContainerContextMenu}>
        {isLoading ? <div className="h-full flex items-center justify-center"><Icons.Grid className="w-10 h-10 animate-spin" /></div>
        : error && !items.length ? (
          <div className="h-full flex flex-col items-center justify-center space-y-4">
             <div className="text-red-500 font-bold uppercase italic text-sm">Access Denied</div>
             <p className="text-[10px] text-gray-500">{(error as any).message || 'This folder is encrypted.'}</p>
             <div className="flex space-x-2"><button onClick={() => setCurrentFolderId(null)} className="border-2 border-black px-4 py-2 text-xs font-bold hover:bg-yellow-400 uppercase">Root</button><button onClick={() => setActiveModal('password-enter')} className="bg-black text-white px-4 py-2 text-xs font-bold hover:bg-yellow-400 uppercase border-2 border-black">Retry</button></div>
          </div>
        ) : sortedItems.length === 0 ? <div className="h-full flex flex-col items-center justify-center opacity-30 uppercase font-bold italic">Empty</div>
        : viewMode === 'grid' ? (
          <GridView 
            ...
            onShare={(item) => { setTargetItem(item); setActiveModal('share'); }}
          />
        ) : (
          <ListView 
            items={sortedItems} selectedIds={selectedIds}
      ...
      {contextMenu && (
        <ContextMenu 
          ...
          onToggleLock={() => { if (contextMenu.item.isLocked) { setTargetItem(contextMenu.item); setActiveModal('password-unlock'); } else { toggleLock.mutate({ id: contextMenu.item.id, isLocked: true }); } setContextMenu(null); }}
          onDelete={() => { setTargetItem(contextMenu.item); setActiveModal('delete'); setContextMenu(null); }}
          onShare={() => { setTargetItem(contextMenu.item); setActiveModal('share'); setContextMenu(null); }}
        />
      )}
