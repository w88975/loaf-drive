...
#### 功能概述
API Key 认证页面，用户输入 API Key 后才能访问网盘功能。这是应用的入口守卫。
...
3. **极客新丑风设计**
   ...
   - 硬核阴影 (`shadow-[10px_10px_0px_0px_rgba(0,0,0,1)]`)
   - 黄色点睛 (`bg-yellow-400`)
   - 全大写字体
   - 背景网格装饰
...
│   ├── 图标（黑色背景 + 黄色图标）
│   ├── 标题 "GEEK.DRIVE"
│   └── 副标题 "Secure Storage Protocol v1"
├── 认证表单
│   ├── 标题栏（黄色背景）
...
**边框与阴影**：
- 主容器：`border-4 border-black`
- 硬核阴影：`shadow-[10px_10px_0px_0px_rgba(0,0,0,1)]`
- Logo 阴影：`shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]`

**按钮状态**：
- 基础态：黑底白字
- 悬停态：`hover:bg-yellow-400 hover:text-black`
- 按下态：`active:scale-95`
- 禁用态：`opacity-30`

#### 响应式设计
- 移动端：全屏显示，表单最大宽度 `max-w-md`
- 桌面端：居中显示，保持最大宽度
- 背景网格：固定定位，装饰性
...
| 特性 | AuthView | FilesView | ShareView | SharesManagementView | TrashView |
|------|----------|-----------|-----------|---------------------|-----------|
| 需要认证 | ❌ | ✅ | ❌ | ✅ | ✅ |
| 文件夹导航 | - | ✅ | ✅ | ❌ | ❌ |
| 加密支持 | - | ✅ | ✅（密码验证） | - | ❌ |
