...
## Purpose
定义文件预览弹框（PreviewModal）的响应式布局行为，确保在不同设备上提供最佳的用户体验：
- PC端（≥768px）：居中弹框模式，保持专业的桌面交互体验
- 移动端（<768px）：全屏沉浸式模式，最大化内容显示区域

本规范确保预览功能在所有屏幕尺寸下都能正常工作，同时遵循项目的 Geek-Brutalism 设计风格。
## Requirements
