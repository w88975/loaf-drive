...
- **进度条**：
  ...
  - 悬停时显示拖动手柄（黑色方块 + 硬核阴影）
  - 黑色边框分隔设计
- **时间显示**：等宽字体显示当前时间/总时长（MM:SS 格式）
- **音量控制**：
  - 静音/取消静音按钮
  - 悬停展开音量滑块
  - 自定义滑块样式（黑色方形手柄 + 硬核阴影）
- **全屏切换**：支持全屏播放
- **自动隐藏控制栏**：播放时 3 秒后自动隐藏，鼠标移动时重新显示

#### UI 风格特性（极客新丑风）
- **粗黑边框**：所有控制按钮使用 `border-2 border-black` 或 `border-4 border-black`
- **硬核阴影**：`shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]` 或 `shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]`
- **黄色点睛**：按钮悬停时 `bg-yellow-400`，进度条使用黄色
- **无圆角设计**：所有元素保持方正
- **等宽字体**：时间显示使用 `font-mono` 
- **高对比度**：纯黑按钮 + 纯白背景 + 黄色进度条
- **按压反馈**：`active:scale-95` 提供点击反馈
...
- **内存优化**：
  ...
  - 后向缓冲保留 5 秒
  - 缓冲上限 30MB
- **自动播放**：可选自动播放（autoplay 时静音）
- **点击播放**：点击 Canvas 区域切换播放/暂停
